...
if(isset($_POST['guardar'])){
    if(isset($_POST['descripcion']) && !empty($_POST['descripcion'])){
        ...
        if($tipo_movimiento == 2){
            $id = $_POST['id'];
            $tipo_movimiento = 2;
            $estado = $clMto_Parentesco->modificar_parentesco($id,$descripcion);
            $mensaje = "Modificado satisfactoriamente";
        }else{
            $tipo_movimiento = 1;
            $estado = $clMto_Parentesco->guardar_parentesco($descripcion);
			if($estado > 0){
    ...
    $list = $clMto_Parentesco->getParentescoByCod($codigo);
    
    if (count($list) > 0) {
        foreach ($list as $row):
        $id = $row['ID_PARENTESCO'];
       
        $desc_parentesco = $row['DESCRIPCION_PARENTESCO'];
        endforeach;      
        $tipo_movimiento = 2;
    }else{
        $tipo_movimiento = 0;
...
<head>
    ...
    <title>Ingresar Parentesco</title>
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">ingresar parentesco</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            Ver todos los parentesco <a class="btn btn-primary btn-circle" href="parentesco.php"><i class="fa fa-table"></i></a> 
                        </div>
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-lg-6">
                                    <form role="form" action="parentesco_ingresar.php" method="POST">                                                        
                                        ...
                                        <div class="form-group">
                                            <label>DESCRIPCIÓN PARENTESCO</label>
                                            <input name="descripcion" required class="form-control" value="<?php echo $desc_parentesco; ?>" placeholder="Descripción detallada">
                                        </div>                                        

...
if(isset($_POST['guardar'])){
    if(isset($_POST['descripcion']) && !empty($_POST['descripcion'])){
        ...
        $nom_editorial = $_POST['descripcion'];
        $tipo_movimiento = $_POST['guardar'];
        if($tipo_movimiento == 2){
            $id = $_POST['id'];
            $tipo_movimiento = 2;
            $estado = $clMto_editorial->modificar_editorial($id,$nom_editorial);
            $mensaje = "Modificado satisfactoriamente";
        }else{
            $tipo_movimiento = 1;
            $estado = $clMto_editorial->guardar_editorial($nom_editorial);
    ...
    $list = $clMto_editorial->geteditorialByCod($codigo);
    
    if (count($list) > 0) {
        foreach ($list as $row):
        $id = $row['CODIGO_EDITORIAL'];
        
        $nom_editorial = $row['NOMBRE_EDITORIAL'];
        endforeach;      
        $tipo_movimiento = 2;
    }else{
        $tipo_movimiento = 0;
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            Ver todos los registros de editorial <a class="btn btn-primary btn-circle" href="editorial_libros.php"><i class="fa fa-table"></i></a> 
                        </div>
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-lg-6">
                                    <form role="form" action="editorial_nuevo.php" method="POST">                                                        
                                        ...
                                        <div class="form-group">
                                            <label>NOMBRE_EDITORIAL</label>
                                            <input name="descripcion" required class="form-control" value="<?php echo $nom_editorial; ?>" placeholder="Descripción detallada">
                                        </div>
